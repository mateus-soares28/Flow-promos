...
start=s.index('    simulateConnectionSuccess: protectedProcedure')
end=s.index('    saveConfig: protectedProcedure', start)
s=s[:start]+s[end:]
marker='''      }),\n  }),\n  // Disparos e Fila'''
...
p.write_text(s)
PY
python3 /home/ubuntu/flowpromos/scripts/patch_router.py
